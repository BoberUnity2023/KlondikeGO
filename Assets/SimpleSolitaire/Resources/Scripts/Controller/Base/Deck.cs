...
namespace SimpleSolitaire.Controller
{
    public abstract class Deck : MonoBehaviour, IPointerClickHandler
    {
        ...
        [SerializeField] protected GameManager _gameManagerComponent;        
        /// <summary>
        ...
        public virtual void SetPositionFromCard(Card card, float x, float y)
        {
            ...
            for (int j = i; j < CardsArray.Count; j++)
            {
                ...
                //Debug.Log("SetPositionFromCard: " + CardsArray[j].name);
            }
        }
        ...
        public virtual List<Card> SetCardsToTop(Card card)
        {
            ...
            for (int i = 0; i < CardsArray.Count; i++)
            {
                ...
                if (found)
                {
                    ...
                    //Debug.Log("SetCardsToTop(" + CardsArray[i].gameObject.name + ")");
                }
            }
            return cards;
        }
        ...
        public bool OverlapWithCard(Card card)
        {
            ...
            float scale = 1080 / _gameManagerComponent.ScreenController.CanvasScaler.referenceResolution.y;

            bool bOverlaped = false;
            float x1 = transform.position.x;
            float x2 = x1 + CardLogicComponent.DeckWidth * scale;
            ...
            float y2 = y1 + CardLogicComponent.DeckHeight * scale;

            float x11 = card.transform.position.x;
            float x21 = x11 + CardLogicComponent.DeckWidth * scale;
            float y11 = card.transform.position.y;
            float y21 = y11 + CardLogicComponent.DeckHeight * scale;
            ...
            //if (bOverlaped)
            //    Debug.LogWarning("OverlapWithCard " + gameObject.name + " & " + card.gameObject.name + "; " + bOverlaped);
            return bOverlaped;
        }
