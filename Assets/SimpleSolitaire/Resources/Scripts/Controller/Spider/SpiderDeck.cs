            ...
            if (Type == DeckType.DECK_TYPE_ACE || Type == DeckType.DECK_TYPE_WASTE)
            {
                ...
                if (HasCards)
                {
                    ...
                    for (int i = CardsArray.Count - 1; i >= 0; i--)
                    {
                        ...
                        {
                            //card.gameObject.SetActive(false);//BB
                        }
                    }
                }
            }
        ...
        public override void UpdateCardsPosition(bool firstTime)
        {
            ...
            for (int i = 0; i < CardsArray.Count; i++)
            {
                ...
                if (Type == DeckType.DECK_TYPE_PACK)
                {
                    ...
                    //card.gameObject.transform.position = gameObject.transform.position - new Vector3(packHorizontalSpace * dealNum, 0, 0);
                    Vector3 curPos = gameObject.transform.position - new Vector3(packHorizontalSpace * dealNum, 0, 0);
                    
                    if (firstTime)
                    {
                        float delay = 2.0f * i / CardsArray.Count;
                        card.MoveTransformTo.SetMoveTo(curPos, 2.0f, delay, true);
                    }
                    else
                ...
                else
                {
                    ...
                    {                        
                        if (firstTime)
                        {
                            ...
                            float delay = 2.0f * i / CardsArray.Count;
                            card.MoveTransformTo.SetMoveTo(curPos, 2.0f, delay, false);
                        }
                        else
                    ...
                    else
                    {
                        if (firstTime)
                        {
                            card.CardStatus = 0;// 0 - задником, 1 - Откр. карты
                        }
                    }
            ...
            {
                //Debug.Log("UpdateCardsPosition(" + firstTime + ") DECK_TYPE_BOTTOM: " + DeckNum); 
                ...
                for (int i = 0; i < CardsArray.Count; i++)
                {
                    ...
                    bool pressed = prevCard != null && !prevCard.IsDraggable && CardsArray.Count > 20 && _gameManagerComponent.ScreenController.Orientation == Screen.ScreenOrientation.Horizontal;

                    var space = prevCard != null && prevCard.CardStatus == 1 && !pressed
                    ...
                    Vector3 finishPosition = deckPos; 
                    if (prevCard != null)
                    {
                        finishPosition = firstTime ? prevCard.MoveTransformTo.FinishPosition : prevCard.gameObject.transform.position;
                    }
                    
                    var prevPos = prevCard != null ? finishPosition : deckPos;

                    var curPos = prevPos - new Vector3(0, space, 0) * spaceMultiplier;

                    if (firstTime)
                    {
                        float delay = 2.0f * i / CardsArray.Count + (DeckNum - 8) * (2.0f / CardsArray.Count / 10);
                        card.MoveTransformTo.SetMoveTo(curPos, 2.0f, delay, true);
                    }
                    else
                    {
                        //Debug.Log("Deck: " + DeckNum + "; card: " + card.gameObject.name + " gameObject.transform.position: ");
                        card.gameObject.transform.position = curPos; 
                    }                   
                }
            }
