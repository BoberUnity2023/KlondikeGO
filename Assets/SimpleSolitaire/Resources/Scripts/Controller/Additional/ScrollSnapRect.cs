...
namespace SimpleSolitaire.Controller.Additional
{
    ...
    public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        [Tooltip("Threshold _time for fast swipe in seconds")]
        public float fastSwipeThresholdTime = 0.3f;
        [Tooltip("Threshold _time for fast swipe in (unscaled) pixels")]
        ...
        private void LerpToPage(int aPageIndex)
        {
            ...
            {
                _points[i].SetActive(i == _currentPage);
            }
        }
        ...
        public void OnEndDrag(PointerEventData aEventData)
        {
            ...
            {
                // if not fast _time, look to which page we got to
        ...
        public void OnDrag(PointerEventData aEventData)
        {
            if (!_dragging)
            {
                // dragging started
                _dragging = true;
                // save _time - unscaled so pausing with Time.scale should not affect it
